...
beautiful.init("/home/overmind/.config/awesome/theme.lua")
...
local cpu = lain.widget.cpu({
	 ...
	 cpu = cpu_now.usage
	 if cpu <= 30 then
	    widget:set_markup("C: <span color='#00FF00'>" .. cpu_now.usage .. "</span> ")
	 elseif cpu > 30 and cpu < 50 then
	    widget:set_markup("C: <span color='#FFFF00'>" .. cpu_now.usage .. "</span> ")
	 elseif cpu > 50 then
	    widget:set_markup("C: <span color='#FF0000'>" .. cpu_now.usage .. "</span> ")
	 end
      end
})
...
local memory = lain.widget.mem({
    settings = function()
		mem = mem_now.used
		if mem >= 6000 then
		   widget:set_markup("M: <span color='#FF0000'>" .. mem_now.used .. "</span> ")
		elseif mem < 6000 and mem >= 5000 then
			widget:set_markup("M: <span color='#FFFF00'>" .. mem_now.used .. "</span> ")
		else
			widget:set_markup( "M: " .. mem .. " ")
		end
    end
})
...
tempwidget:set_text("T: ~")
tempwidgettimer = timer({ timeout = 5 })
tempwidgettimer:connect_signal("timeout",
			       function()
				  status = io.popen("cat /proc/acpi/ibm/thermal | awk '{print $2}'", "r")
				  t = tonumber(status:read())
				  if t > 50 and t < 69 then
				     tempwidget:set_markup("T: <span color='#FFFF00'>"..t.. "&#x2103;</span>")
				  end
				  
				  if t >= 70 then
				     tempwidget:set_markup("T: <span color='#FF0000'>"..t.. "&#x2103;</span>")
				  end
				  
				  if t <= 50 then
				     tempwidget:set_markup("T: <span color='#00FF00'>"..t.. "&#x2103;</span>")
				  end
				  status:close()
			       end
)
...
vpnwidgettimer:connect_signal("timeout",
  function()
    status = io.popen("ip tuntap show | grep ".. config.vpn_device , "r")
...
--mytextclock = wibox.widget.textclock({format="%a %b %e %l:%M%P"})
...
awful.screen.connect_for_each_screen(function(s)
      ...
      awful.tag({ "term", "emacs", "web", "mail", "im", "docs", "other" }, s, awful.layout.layouts[1])
...
globalkeys = awful.util.table.join(
   ...
   awful.key({}, "XF86AudioMicMute", function() os.execute("pactl set-source-mute 1 toggle") end),

   -- screenshot

   awful.key({modkey, "Shift"}, "r", function() awful.util.spawn_with_shell("sleep 0.5 && scrot -s -e 'mv $f ~/Screenshots/'") end,
      {description = "Make screenshot with scrot", group = "awesome"})
)
